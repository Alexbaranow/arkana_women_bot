      ...
      <main>
        {loading ? (
          <div className="my-readings-loading">
            <TarotShuffleLoader size={48} />
            <p className="profile-subtext">Загрузка…</p>
          </div>
        ) : (
          <>
            <section
              className="my-readings-section"
              data-aos="fade-up"
            >
              <h2 className="profile-section-title">
                Ожидают оплату / В работе
              </h2>
              {awaiting.length === 0 ? (
                ...
                <ul className="my-readings-list">
                  {awaiting.map((o) => (
                    <li
                      key={o.id}
                      className="my-readings-order-card card"
                    >
                      <div className="my-readings-order-header">
                        <span className="my-readings-order-title">
                          {o.product_title}
                        </span>
                      ...
                      <p className="my-readings-order-meta">
                        {o.price_rub} ₽
                        {o.paid_at && (
                          <> · Оплачен {formatOrderDate(o.paid_at)}</>
                        )}
                        {!o.paid_at && o.created_at && (
                          <> · Создан {formatOrderDate(o.created_at)}</>
                        )}
                      </p>
                    </li>
                  ))}
              ...
              {delivered.length === 0 ? (
                ...
                <ul className="my-readings-list">
                  {delivered.map((o) => (
                    <li
                      key={o.id}
                      className="my-readings-order-card card my-readings-order-delivered"
                    >
                      <div className="my-readings-order-header">
                        <span className="my-readings-order-title">
                          {o.product_title}
                      ...
                      <p className="my-readings-order-meta">
                        {o.price_rub} ₽ ·{" "}
                        {formatOrderDate(o.paid_at || o.created_at)}
                      </p>
                      {o.result_text && (
                        <div className="my-readings-order-description">
                          {o.result_text}
                        </div>
                      )}
                    </li>
